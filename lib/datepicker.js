...
Datepicker = (function(_super) {
  ...
  Datepicker.prototype._show = function() {
    ...
    if (val) {
      this.date = moment(val, this.opts.format);
    }
    this.monthOnly = this.opts.monthpicker;
    this._renderPanel();
  ...
  Datepicker.prototype._renderPanel = function() {
    ...
    this._monthpicker = this.cal.find('.datepicker-yearmonth').data('tmpDate', this.date.clone().startOf('month'));
    this.title = this.cal.find('.datepicker-title');
    this.yearContainer = this.cal.find('.datepicker-year-container');
    if (this.opts.monthpicker) {
      this._monthpicker.show();
      year = this.date.year();
      return this.yearContainer.scrollTop(20 * (year - this.firstYear));
    } else {
    ...
    this.cal.on('mousedown click', function(e) {
      ...
      return function(e) {
        ...
        if (!_this.opts.monthpicker) {
          _this._monthpicker.toggle();
        }
        year = _this.date.year();
        return _this.yearContainer.scrollTop(20 * (year - _this.firstYear));
      };
    ...
    return this.cal.find('.datepicker-year-container').scroll((function(_this) {
      return function(e) {
        ...
        if (scrollTop + 80 + 10 >= _this.yearContainer.children().height()) {
          _this.yearContainer.find('.datepicker-year-list').append(_this._renderYearSelectors(_this.lastYear + 1, _this.lastYear + 5));
          return _this.lastYear = _this.lastYear + 10;
        } else if (scrollTop === 0) {
          _this.yearContainer.find('.datepicker-year-list').prepend(_this._renderYearSelectors(_this.firstYear - 5, _this.firstYear - 1));
          _this.firstYear = _this.firstYear - 5;
          return _this.yearContainer.scrollTop(20 * 5);
        }
      };
    })(this));
  };
  ...
  Datepicker.prototype._refresh = function() {
    ...
    this.title.text(this._formatTitle(this.date));
    year = this.date.year();
    ...
    return this._monthpicker.find("[data-month=" + month + "]").addClass('selected');
  };
  ...
  Datepicker.prototype._renderCal = function() {
    ...
    return "<table class=\"calendar\">\n  <tr class=\"datepicker-dow\">\n    " + week + "\n  </tr>\n  " + (this._renderDaySelectors(this.date)) + "\n</table>";
  };

  Datepicker.prototype._renderYearMonth = function() {
    var currentMonth, currentYear, noSelected, title;
    noSelected = this.opts.monthOnly && !this.selectedDate;
    title = noSelected ? '&nbsp;' : this._formatTitle(this.date);
  ...
  Datepicker.prototype.setSelectedDate = function(date) {
    if (date) {
      this.date = moment(date, this.opts.format);
      this._refresh();
    }
