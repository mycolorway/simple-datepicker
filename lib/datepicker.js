(function() {
  ...
  Datepicker = (function(_super) {
    ...
    Datepicker.prototype._init = function() {
      ...
      val = this.el.val();
      if (val) {
        this.selectedDate = moment.tz(val, this.opts.format, simple.tz);
      }
      return this._render();
    };
    ...
    Datepicker.prototype.update = function(date) {
      ...
      if (!this.cal) {
        ...
        this.cal.on("mousedown", function(e) {
          ...
          return function(e) {
            ...
            return _this.cal.trigger("select.datepicker", [date, btn]);
          };
        })(this));
      }
      if (this.opts.width) {
      ...
      return this.cal.trigger("beforeUpdate.datepicker", [this.cal]);
    };
        ...
        while (x < 7) {
          ...
          if (n >= 1 && n <= lastDate.date()) {
            ...
            if (moment.isMoment(this.opts.disableBefore)) {
              until_ = moment.tz(this.opts.disableBefore, "YYYY-MM-DD", simple.tz);
              c += (date.diff(until_) < 0 ? " disabled" : "");
            }
            if (moment.isMoment(this.opts.disableAfter)) {
              until_ = moment.tz(this.opts.disableAfter, "YYYY-MM-DD", simple.tz);
              c += (date.diff(until_) > 0 ? " disabled" : "");
            }
    ...
    Datepicker.prototype.setSelectedDate = function(date) {
      if (!date) {
        ...
        this.selectedDate = moment.tz(date, simple.tz);
