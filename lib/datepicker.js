...
Datepicker = (function(superClass) {
  ...
  Datepicker.prototype._bindMouse = function() {
    ...
    this.cal.on('click', '.panel-year p', (function(_this) {
      return function(e) {
        ...
        if (['prev', 'next'].indexOf(year) === -1) {
          ...
          _this._year.val(year);
          _this._yearSelector.find('.selected').removeClass('selected');
          _this._yearSelector.find("[data-year=" + year + "]").addClass('selected');
          return _this._month.focus();
        } else {
          from = _this._yearSelector.find('p:not(.menu-item)').eq(0).data('year');
    ...
    this.cal.on('click', '.panel-month p', (function(_this) {
      return function(e) {
        ...
        _this._monthSelector.find('.selected').removeClass('selected');
        _this._monthSelector.find("[data-month=" + month + "]").addClass('selected');
        if (!_this.opts.monthpicker) {
          _this._calendar.replaceWith(_this._renderCal());
          _this._calendar = _this.cal.find('.panel-day');
        }
        _this._month.val(Number(month) + 1);
        if (_this.opts.monthpicker) {
    ...
    this.cal.on('click', '.panel-day a', (function(_this) {
      return function(e) {
        var btn;
        e.preventDefault();
        btn = $(e.currentTarget);
        ...
        _this._day.val(_this.date.date());
        return _this._select();
      };
    })(this));
    return this.cal.on('click', '.panel-day .menu-item', (function(_this) {
      return function(e) {
        var $target, direction;
        $target = $(e.currentTarget);
        direction = $target.is('.prev') ? -1 : 1;
        _this.date.add('month', direction);
        _this._refresh(true);
        return _this.cal.find('.panel-day').addClass('active');
      };
    })(this));
  };

  Datepicker.prototype._bindKey = function() {
    this.cal.on('keydown', '.datepicker-header input', (function(_this) {
      return function(e) {
        ...
        if (key === 9) {
          _this._update();
          $next = e.shiftKey ? $input.prevAll('input:first') : $input.nextAll('input:first');
          if ($next.length) {
            $next.focus();
          } else {
            _this._select();
    ...
    return this.cal.on('input', 'input', (function(_this) {
      return function(e) {
        var $input, max, value;
        $input = $(e.currentTarget);
        value = $input.val();
        switch ($input.data('type')) {
          ...
          case 'day':
            max = _this.date.endOf('month').date();
            if (Number(value) > max) {
              return $input.val(value.substr(1));
            }
        }
      };
    })(this));
  ...
  Datepicker.prototype._update = function() {
    ...
    firstYear = this._yearSelector.find('p:not(.menu-item)').data('year');
    if (year < firstYear || year > firstYear + 11) {
      firstYear = year - (year % 10);
      this._yearSelector.replaceWith(this._renderYearSelector(firstYear));
      this._yearSelector = this.cal.find('.panel-year');
    }
    this.cal.find('.selected').removeClass('selected');
    this._yearSelector.find("[data-year=" + year + "]").addClass('selected');
    this._monthSelector.find("[data-month=" + month + "]").addClass('selected');
    return this._calendar.find("[data-date=" + (this.date.format('YYYY-MM-DD')) + "]").addClass('selected');
  };

  Datepicker.prototype._refresh = function(all) {
    if (all == null) {
      all = false;
    }
    if (all) {
    ...
    this._calendar = this.cal.find('.panel-day');
    this.cal.find('.selected').removeClass('selected');
    this._yearSelector.find("[data-year=" + (this.date.year()) + "]").addClass('selected');
    this._monthSelector.find("[data-month=" + (this.date.month()) + "]").addClass('selected');
    return this._calendar.find("[data-date=" + (this.date.format('YYYY-MM-DD')) + "]").addClass('selected');
  };
  ...
  Datepicker.prototype._renderDayMenu = function() {
    return "<div class=\"menu-item prev\"><i class=\"icon-chevron-left\"><span>&lt;</span></i></div>\n<div class=\"menu-item next\"><i class=\"icon-chevron-left\"><span>&gt;</span></i></div>";
  };
  ...
  Datepicker.prototype._renderHeader = function() {
    ...
    if (!this.opts.monthpicker) {
      el += '<input type="text" class="day-input" data-type="day" data-min="1"><span>日</span>';
    }
    return el;
  };
  ...
  Datepicker.prototype._renderYearSelector = function(from) {
    ...
    el += '<p class="datepicker-year menu-item" data-year="prev"><i class="icon-chevron-left"><span>&lt;</span></i></p><p class="datepicker-year menu-item" data-year="next"><i class="icon-chevron-right"><span>&gt;</span></i></p>';
    for (year = j = ref = from, ref1 = from + 11; ref <= ref1 ? j <= ref1 : j >= ref1; year = ref <= ref1 ? ++j : --j) {
