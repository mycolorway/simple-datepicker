(function() {
  ...
  Datepicker = (function(_super) {
    ...
    Datepicker.prototype._init = function() {
      ...
      this.el = this.opts.el.data("datepicker", this);
      val = this.el.val();
      if (val) {
    ...
    Datepicker.prototype._update = function() {
      var calendar, newDate, num, offset, theDate, today, _i, _ref;
      today = moment().startOf("day");
      theDate = this.el.data("theDate") || this.selectedDate || today;
      this.el.data("theDate", theDate);
      if (this.opts.monthCount > 1) {
        for (num = _i = _ref = this.opts.monthCount; _ref <= 1 ? _i <= 1 : _i >= 1; num = _ref <= 1 ? ++_i : --_i) {
          newDate = theDate.clone().add("months", 1 - num);
          calendar = calendar ? calendar + this._renderCal(newDate) : this._renderCal(newDate);
        }
      } else {
        ...
        this.cal.on("mousedown", function(e) {
          ...
          return function(e) {
            ...
            if (_this.opts.onSelect) {
              return _this.opts.onSelect(date);
            }
          };
        })(this));
      }
      ...
      if ($.isFunction(this.opts.onUpdate)) {
        return this.opts.onUpdate(this.cal);
      }
    };
